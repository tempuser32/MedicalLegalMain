document.addEventListener('DOMContentLoaded', function() {
    ...
    // Generate sample consent requests if none exist
    generateSampleConsents();
...
function generateSampleConsents() {
    ...
    // Check if any records have consent status
    const hasConsentStatus = allMedicalRecords.some(record => record.consentStatus);
    
    if (!hasConsentStatus) {
        // Add consent status to each record
        allMedicalRecords.forEach((record, index) => {
            // Assign different statuses for demonstration
            if (index % 3 === 0) {
                record.consentStatus = 'approved';
                record.consentDate = new Date().toISOString();
                record.consentExpiryDate = new Date(Date.now() + 180 * 24 * 60 * 60 * 1000).toISOString(); // 180 days
            } else if (index % 3 === 1) {
                record.consentStatus = 'pending';
            } else {
                record.consentStatus = 'rejected';
                record.consentDate = new Date().toISOString();
            }
        });
