document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('Current patient name:', patientName);
...
function loadConsentsAndRequests(patientName) {
    ...
    console.log('All access requests:', accessRequests);
    
    // Fix: Make patient name comparison case-insensitive and more flexible
    const pendingRequests = accessRequests.filter(request => {
        // Check if patient names match (case insensitive)
        const nameMatches = request.patientName && 
                           patientName && 
                           request.patientName.toLowerCase() === patientName.toLowerCase();
        
        console.log(`Request ${request.id}: Name match: ${nameMatches}, Status: ${request.status}, Requires consent: ${request.requiresPatientConsent}`);
        
        return nameMatches && 
               request.status === 'pending' && 
               request.requiresPatientConsent !== false; // Accept true or undefined
    });
    
    console.log('Pending requests for patient:', pendingRequests);
    
    // Convert pending requests to consent format
    const pendingConsents = pendingRequests.map(request => ({
        id: request.id,
        title: `Access Request for ${request.caseType || 'Medical'} Case`,
        status: 'pending',
        requestedBy: request.legalProfessionalName || 'Legal Professional',
        organization: request.legalProfessionalEmail ? request.legalProfessionalEmail.split('@')[1] : 'Legal Organization',
        purpose: request.reason || 'Medical record access',
        requestDate: request.timestamp || new Date().toISOString(),
        ...
        caseType: request.caseType || 'Medical',
    ...
    console.log('Pending consents:', pendingConsentRequests);
    console.log('Expired consents:', expiredConsents);
...
function loadConsentsByType(type, consents) {
    ...
    if (!consentList || !emptyState) {
        console.error(`Elements for ${type} consents not found`);
        return;
    }
